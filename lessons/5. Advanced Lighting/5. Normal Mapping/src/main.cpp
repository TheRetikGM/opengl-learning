...
{	
	...
	if (glfwGetKey(window, GLFW_KEY_F1) == GLFW_PRESS)			// F1
	{
		...
		{
			std::cout << INFO << " Polygon mode: " << YELLOW << "fill" << DEFAULT << "\n";
			polygonMode = GL_FILL;
		}
	}
	else if (glfwGetKey(window, GLFW_KEY_F2) == GLFW_PRESS)			// F2
	{
		...
		{
			std::cout << INFO << " Polygon mode: " << YELLOW << "line" << DEFAULT << "\n";
			polygonMode = GL_LINE;
		}
	}
	if (glfwGetKey(window, GLFW_KEY_F3) == GLFW_PRESS)			// F3
	{
		...
		std::cout << INFO << " Screenshot saved to '" MAGNETA << path << DEFAULT "'\n";
	}	
	//if (glfwGetKey(window, GLFW_KEY_F4) == GLFW_PRESS);			// F4		
	if (glfwGetKey(window, GLFW_KEY_1) == GLFW_PRESS)			// 1
	{
		if (!normalMapping)
			std::cout << INFO << " Normal mapping: " << GREEN << "on" << DEFAULT << "\n";
		normalMapping = true;
	}
	else if (glfwGetKey(window, GLFW_KEY_2) == GLFW_PRESS)			// 2
	{
		if (normalMapping)
			std::cout << INFO << " Normal mapping: " << RED << "off" << DEFAULT << "\n";
		normalMapping = false;
	}
	/* -------------------- Movement -------------------- */
...
void draw_scene(const Shader* shader, bool lightCube)
{
	...
	if (cyborg == NULL)
		cyborg = new glml::Model(MODELS_DIR "cyborg/cyborg.obj", false, true);
	...
	model = glm::translate(glm::mat4(1.0f), glm::vec3(0.0f, -2.0f, -2.0f));
